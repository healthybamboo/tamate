...
- 作成時に待機時間（1分 / 10分 / 1時間 / 1日）を選択
- 「開く」から始まる待機と、1秒ごとに更新されるカウントダウン
- アプリを閉じている間も進む待機と、復帰・再起動時の状態の復元
- 解錠から5分後の再ロック
- 解錠時刻のローカル通知と、通知タップで対象のメモを開くこと

解錠条件は `UnlockRule` として抽象化してあり、待機時間以外の仕掛け（パズル、場所、回数など）を
足すときは実装を1つ増やして `UnlockRule.fromJson` に分岐を書けばよい。
