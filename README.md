...
| フォント | `google_fonts` | Zen Kaku Gothic New を実行時に取得 |
| Lint | `flutter_lints` + `riverpod_lint` | |

## 実装済みのもの

- メモの一覧・作成・削除、解錠中のみの編集
- 解錠のしかたを3つから選択（待つ / 待つ + 問いかけ / 問いかけだけ）
- 待機時間（1分 / 3分 / 5分 / 10分）の選択。解錠中なら後から変更できる
- 開く前に自分へ投げる問いを複数登録。順に答えて、すべて「はい」なら開く。「いいえ」なら引き返す
- 「開く」から始まる待機と、1秒ごとに更新されるカウントダウン
- 待機画面を見ている間だけ進む待機（離れると最初からやり直し）
- 解錠から5分後の再ロック
- 4桁のコードの生成（スクリーンタイムなど外部のパスコード用。保存すると読み直すには待つ）
- 開封した回数と日時、問いで引き返した回数の記録。日付 × 時刻のグラフでの振り返り
